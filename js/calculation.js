...
//if function returns true, there were no errors; create empty table
  ...
  for(var i = 1; i < row_range; i++){
    for(var j = 1; j < column_range; j++){
      var temp1 = tbl.rows[0].cells[j].innerHTML;
      var temp2 = tbl.rows[i].cells[0].innerHTML;
      var product = temp1 * temp2;
      tbl.rows[i].cells[j].innerHTML = product;
    }
  }
}

//saves the table to a tab when user presses save button
function save_table() {
  document.getElementById("table-1").innerHTML = document.getElementById("myTable").innerHTML;
